...
impl XcodeProductKind {
	fn built_products_filename(self, output_basename: &str) -> String {
		match self {
			...
			Self::DynamicLibrary => format!("{}.dylib", output_basename),
...
fn new_native_target_archive(
	...
	let (dependencies, build_phases, build_rules, _has_shared_runtime_deps) = new_native_target_common(
		target,
		sources,
		generator_vars,
		toolchain,
...
fn clone_xc_with(
	...
	for build_cfg in native_target_build_configs {
		...
		match product_kind {
			XcodeProductKind::DynamicLibrary => {
				ensure_dylib_install_name(&mut build_settings, product_kind, &product_output_name);
				if add_runtime_runpaths {
					append_deduplicated_runpaths(&mut build_settings, &["@loader_path"]);
				}
			}
			XcodeProductKind::Executable if add_runtime_runpaths => {
				append_deduplicated_runpaths(&mut build_settings, &["@executable_path"]);
			}
			XcodeProductKind::Executable => {}
			XcodeProductKind::StaticArchive => {}
		}
...
fn ensure_dylib_install_name(
	build_settings: &mut BTreeMap<String, BuildSetting>,
	product_kind: XcodeProductKind,
...
fn append_deduplicated_runpaths(build_settings: &mut BTreeMap<String, BuildSetting>, additional_runpaths: &[&str]) {
	let mut runpaths = existing_runpaths(build_settings);
	for runpath in additional_runpaths {
		runpaths.insert((*runpath).to_owned());
	}
	build_settings.insert("LD_RUNPATH_SEARCH_PATHS".to_owned(), BuildSetting::Array(runpaths.into_iter().collect()));
}

fn existing_runpaths(build_settings: &BTreeMap<String, BuildSetting>) -> IndexSet<String> {
