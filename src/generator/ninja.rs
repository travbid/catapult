	...
	pub fn generate(
		...
		let star_context = StarContext {
			c_compiler: toolchain
				.c_compiler
				.as_ref()
				.map(|compiler| StarContextCompiler { arch: compiler.target() }),
...
fn add_static_lib_target(
	...
	let sources = lib
		.sources
		.extended_with(split_sources(&generator_vars.sources, &lib.project().info.path)?);
...
fn add_executable_target(
	...
	let sources = exe
		.sources
		.extended_with(split_sources(&generator_vars.sources, &exe.project().info.path)?);
